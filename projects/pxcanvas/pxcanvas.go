...
func NewPxCanvas(state *apptype.State, config apptype.PxCanvasConfig) *PxCanvas {
	...
	pxCanvas.PixelData = NewBlankImage(
		pxCanvas.PxCols, pxCanvas.PxRows, color.NRGBA{255, 100, 255, 255},
	)
...
func (pxCanvas *PxCanvas) CreateRenderer() fyne.WidgetRenderer {
	...
	for i := 0; i < len(canvasBorder); i++ {
		canvasBorder[i].StrokeColor = color.NRGBA{100, 100, 100, 100}
...
func (pxCanvas *PxCanvas) TryPan(
	prevCoord *fyne.PointEvent, ev *desktop.MouseEvent,
) {
...
func (pxCanvas *PxCanvas) MouseToCanvas(ev *desktop.MouseEvent) (*int, *int) {
...
func (pxCanvas *PxCanvas) LoadImage(img image.Image) {
	...
	pxCanvas.PxCanvasConfig.PxCols = dims.Dx()
	pxCanvas.PxCanvasConfig.PxRows = dims.Dy()
...
func (pxCanvas *PxCanvas) NewDrawing(cols, rows int) {
	...
	PixelData := NewBlankImage(cols, rows, color.NRGBA{100, 5, 5, 5})
