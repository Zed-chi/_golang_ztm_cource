...
func (renderer *PxCanvasRenderer) LayoutCanvas(size fyne.Size) {
	...
	renderer.canvasImage.Move(
		fyne.NewPos(
			renderer.pxCanvas.CanvasOffset.X,
			renderer.pxCanvas.CanvasOffset.Y,
		),
	)
	renderer.canvasImage.Resize(
		fyne.NewSize(
...
func (renderer *PxCanvasRenderer) LayoutBorders(size fyne.Size) {
	...
	right.Position1 = fyne.NewPos(offset.Y+imgHeight, offset.Y)
	right.Position2 = fyne.NewPos(offset.Y+imgHeight, offset.Y+imgHeight)
